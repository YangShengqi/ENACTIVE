...
class Semantic_Agent(AgentBase):
        ...
        if self.target_type == "without_self":
            hor_list, ver_list, shots_list, targets_list, v_c_list, nn_c_list, lts_list = \
                model.select_action(self.states_global, self.states_native, self.states_tokens,
                                    msl_token_self, msl_token_bandit, state_id_one_hot,
                                    hor_masks, ver_masks, shot_masks, target_masks, self.last_step_decision, lts_masks)
        ...
        with torch.no_grad():
            ...
            for i in range(self.side * env.red, env.red + self.side * env.blue):
                ...
                if self.dones[i]:
                    ...
                    if int(lts_out[i if i < env.red else i - env.red] + 0.1):
                        self.hor_one_hots.append([0] * self.model_action_dims["horizontal_cmd_dim"])
                        self.ver_one_hots.append([0] * self.model_action_dims["vertical_cmd_dim"])
                    else:
        ...
        with torch.no_grad():
            for i in range(batch_forward_times):
                ...
                fixed_log_probs_b, values_b = model.get_log_prob_and_values(states_global_b, states_native_b, states_token_b,
                                                                        ...
                                                                        v_cs_b, nn_cs_b, last_step_decision_b, lts_masks_b, lts_b)
        ...
        for epoch in range(Config.epochs):
            ...
            for i in range(optim_iter_num):
                ...
                hor_prob, ver_prob, shoot_prob, target_prob, v_c_prob, nn_c_prob, _, lts_prob = model.batch_forward(
                    ...
                    target_masks_b,
                    hor_one_hots_b,
                    ver_one_hots_b, last_step_decision_b, lts_masks_b)
    ...
    def last_maneuver_to_one_hot(self):
        ...
        targets_one_hot = self.action_to_one_hot(self.last_targets, 3)
        hors_one_hot = self.action_to_one_hot(self.last_hors, 6)
        vers_one_hot = self.action_to_one_hot(self.last_vers, 7)
        v_cmd_one_hot = self.action_to_one_hot(self.last_v_cmd, 2)
        n_cmd_one_hot = self.action_to_one_hot(self.last_n_cmd, 2)
    ...
    def get_lts_masks(self, env):
        ...
        lts_masks = []
        for i in range(self.team_aircraft_num):
            self_abs_id = i + self.side * env.red
            cur_target_id = self.last_targets[i]
            if cur_target_id == -1:  # not
                lts_masks.append([1.0, 0.0])
            else:
                if self_abs_id <= cur_target_id:
                    cur_abs_target_id = cur_target_id + 1
                else:
                    cur_abs_target_id = cur_target_id
                if self.dones[cur_abs_target_id]:
                    lts_masks.append([1.0, 0.0])
                else:
...
if __name__ == "__main__":
    ...
    red_agent = Semantic_Agent(target_type="without_self", agent_save_mode="origin",
                               reward_type="expert_rewards",
                               reward_hyperparam_dict=origin_reward_parameters)
    blue_agent = Semantic_Agent(target_type="without_self", agent_save_mode="origin",
                                reward_type="expert_rewards",
                                reward_hyperparam_dict=origin_reward_parameters)
