...
# from environment import state_method_independ, reward_method_independ, reward_shaping
...
class Lts_Agent(AgentBase):
    ...
    def train(self, batchs):
        ...
        for epoch in range(Config.epochs):
            ...
            for i in range(optim_iter_num):
                ...
                loss_step(model, model_optimizer, loss)
    ...
    def last_maneuver_to_one_hot(self, env):
        ...
        targets_one_hot = self.action_to_one_hot(self.last_targets, env.red + env.blue - 1)  # default for without self method
        hors_one_hot = self.action_to_one_hot(self.last_hors, env.action_interface["AMS"][self.side * env.red]["SemanticManeuver"]["horizontal_cmd"]["mask_len"])
        vers_one_hot = self.action_to_one_hot(self.last_vers, env.action_interface["AMS"][self.side * env.red]["SemanticManeuver"]["vertical_cmd"]["mask_len"])
    ...
    def _get_delib_cost(self):
        ...
        if self.version < 300:
            ...
            self.deliberation_cost_factor = 0.4 - (self.version - 300) * 0.4 / 200
        else:
            self.deliberation_cost_factor = 0
    ...
    def distillation_train(self, batchs):
        ...
        batch = dict(state_global=[], state_native=[], state_id=[],
                     ...
                     hor_masks=[], ver_masks=[], target_masks=[], shot_masks=[], current_step_available=[],
                     shot_data=[], type=[], final_rewards=[], mask=[], mask_solo_done=[],)
        ...
        expert_targets_prob_s = (expert_targets_exp / torch.sum(expert_targets_exp, dim=-1, keepdim=True))[
            shot_index].clone()
        ...
        with torch.no_grad():
            for i in range(batch_forward_times):
                ...
                values_b, _, _, _, _, _, _ = model.distillation_forward(states_global_b, states_native_b,
                                                      states_token_b,
        ...
        """get advantage estimation from the trajectories"""
        _, returns = estimate_advantages_gae_independ(final_rewards, masks, mask_solo_done, values,
                                                      device=device)
        returns_s = returns[shot_index]
        ...
        while epoch < Config.Distillation.epochs:
            ...
            returns_s = returns_s[perm_s].clone()
            ...
            for i in range(optim_iter_num):
                ...
                all_return_b = torch.cat((returns_b, returns_s_b), dim=0)
                ...
                # agent的输出
                values_pred, hor_probs, ver_probs, shot_probs, target_probs, v_c_probs, nn_c_probs = model.distillation_forward(
                    all_states_global_b,
                    all_states_native_b,
                    all_states_token_b,
                ...
                # value_loss
                value_loss = (values_pred.squeeze(-1) * all_current_step_available_b - all_return_b).pow(2).mean()

                # loss
                loss = supervise_lose + 100 * value_loss
                loss_step(model, model_optimizer, loss)
                # for param in self.model.parameters():
                #     print(param.grad)
                print(epoch, supervise_lose, value_loss, loss)
...
if __name__ == "__main__":
    ...
    red_agent = Lts_Agent(target_type="without_self", agent_save_mode="origin",
