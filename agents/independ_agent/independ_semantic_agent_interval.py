...
# 2020/11/23 copy from independ_semantic_agent.py, add self changing interval method #

from framwork.agent_base import AgentBase
# from models.independ_nn.independ_semantic_nn import Independ_Semantic_NN
from models.independ_nn.independ_semantic_slim_nn import Independ_Semantic_NN
...
class Independ_Semantic_Agent(AgentBase):
    def __init__(self, target_type=None, agent_save_mode=None, reward_type=None, reward_update_type=None, reward_hyperparam_dict=None):
        ...
        self.interval_changing_method = True
        self.dones = None
        ...
        # self.last_step_maneuver = None
        self.last_time_maneuver_horizontal = None
        self.last_time_maneuver_vertical = None

    def _create_model(self):
        red_state_global = state_method_independ.get_kteam_global_ground_truth_state(Config.env, 0)
        red_atten_state = state_method_independ.get_kteam_aircraft_state_for_attention_refactor(Config.env, 0)
        ...
        dim_native = len(red_atten_state[0][0]) + \
                     Config.env.action_interface["AMS"][0]["SemanticManeuver"]["horizontal_cmd"]["mask_len"] + \
                     Config.env.action_interface["AMS"][0]["SemanticManeuver"]["vertical_cmd"]["mask_len"]

        model = Independ_Semantic_NN(Config.env.red, len(red_state_global[0]), dim_native,
        ...
        self.last_time_maneuver_horizontal = []
        self.last_time_maneuver_vertical = []
        for i in range(Config.env.red):
            self.last_time_maneuver_horizontal.append(np.nan)
            self.last_time_maneuver_vertical.append(np.nan)
    ...
    def before_step_for_sample(self, env):
        ...
        # print(self.last_time_maneuver_vertical)
        # print(self.last_time_maneuver_horizontal)

        last_time_maneuver = []
        for i in range(len(self.last_time_maneuver_horizontal)):
            last_time_maneuver.append([])
            cur_horizontal = np.zeros(self.horizontal_cmd_dim).tolist()
            cur_vertical = np.zeros(self.vertical_cmd_dim).tolist()
            if self.last_time_maneuver_horizontal[i] is np.nan:
                pass
            else:
                cur_horizontal[self.last_time_maneuver_horizontal[i]] = 1
            if self.last_time_maneuver_vertical[i] is np.nan:
                pass
            else:
                cur_vertical[self.last_time_maneuver_vertical[i]] = 1
            last_time_maneuver[-1].append(cur_horizontal)
            last_time_maneuver[-1].append(cur_vertical)

        # print("last_time_maneuver", last_time_maneuver)
        ...
        states_atten = state_method_independ.get_kteam_aircraft_state_for_attention_refactor(env, self.side, last_time_maneuver_one_hot=last_time_maneuver)
        ...
        if self.target_type == "without_self":
            hor_list, ver_list, shots_list, targets_list, v_c_list, nn_c_list = model.select_action(self.states_global,
                                                                                                    self.states_native,
                                                                                                    self.states_tokens,
                                                                                                    msl_token_self,
                                                                                                    msl_token_bandit,
        ...
        for i in range(self.side * env.red, env.red + self.side * env.blue):
            if not self.dones[i]:
                ...
                # self.last_time_maneuver_horizontal[i if i < env.red else i - env.red] = cur_hor
                # self.last_time_maneuver_vertical[i if i < env.red else i - env.red] = cur_ver
    ...
    def after_step_for_train(self, env):
        ...
        # print("hor", self.hors, self.last_time_maneuver_horizontal)
        # print("vers", self.vers, self.last_time_maneuver_vertical)
        hors_list = deepcopy(self.hors)
        shape_reward = [
            reward_shaping.get_ith_aircraft_shaped_reward(env, i, self.side, self.interval, reformed_target_list, hors_list, self.hors, self.last_time_maneuver_horizontal, self.vers, self.last_time_maneuver_vertical,
        ...
        # update last time maneuver
        self.last_time_maneuver_horizontal = deepcopy(self.hors)
        self.last_time_maneuver_vertical = deepcopy(self.vers)
        ...
        self.batchs["bandit_msl_token"].append(self.bandit_msk_tokens)  #todo didn't add next state in log
    ...
    def train(self, batchs):
        ...
        for info in Config.interval_auto_decline.interval_decline_info:
            if info["min_version"] <= self.version < info["max_version"]:
                batch_forward_size = info["optim_batch_size"]
                break
        print("optim batch size", batch_forward_size)
        ...
        optim_iter_num = int(states_native.shape[0] / batch_forward_size)
        ...
        self.version += 1
        self._interval_tune()  # tune interval

        return self

    def _interval_tune(self):
        if not self.interval_changing_method:
            pass
        else:
            for info in Config.interval_auto_decline.interval_decline_info:
                if info["min_version"] <= self.version < info["max_version"]:
...
if __name__ == "__main__":
    ...
    red_agent = Independ_Semantic_Agent(target_type="without_self", agent_save_mode="origin",
                                        reward_type="expert_rewards",
                                        reward_hyperparam_dict=origin_reward_parameters)
    blue_agent = Independ_Semantic_Agent(target_type="without_self", agent_save_mode="origin",
                                         reward_type="expert_rewards",
                                         reward_hyperparam_dict=origin_reward_parameters)
