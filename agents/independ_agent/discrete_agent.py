...
from models.independ_nn.independ_discrete_nn import Independ_Discrete_NN
...
class Discrete_Agent(AgentBase):
    def __init__(self, target_type=None, agent_save_mode=None, reward_type=None, reward_update_type=None, reward_hyperparam_dict=None):
        self.batchs = dict(state_global=[], state_native=[], state_id=[],
                           ...
                           maneuver=[], shot=[], target=[], mask=[], mask_solo_done=[],
                           maneuver_masks=[], target_masks=[], shot_masks=[],
                           next_state_global=[], next_state_native=[], next_state_token=[],
                           team_rewards=[], solo_rewards=[], other_mean_reward=[], log=None)
    ...
    def _create_model(self):
        ...
        model = Independ_Discrete_NN(len(red_state_global[0]), len(red_atten_state[0][0]), Config.env.red,
    ...
    def before_step_for_sample(self, env):
        ...
        if self.target_type == "origin" or self.target_type == "without_self":
            maneuver_list, shots_list, targets_list = model.select_action(self.states_global, self.states_native,
                                                                          self.states_tokens, msl_token_self,
                                                                          msl_token_bandit, state_id_one_hot,
    ...
    def after_step_for_train(self, env):
        ...
        solo_sum_rewards = [reward_method.get_ith_aircraft_reward(env, i, self.rewards_hyperparam_dict) for i in range(self.side * env.red, env.red + self.side * env.blue)]
        solo_rewards = [solo_sum_rewards[i] - self.current_episode_solo_reward[i] for i in range(env.blue if self.side else env.red)]
        self.current_episode_solo_reward = solo_sum_rewards

        other_sum_mean_reward = [
            reward_method.get_mean_team_reward_without_ith_aircraft(env, i, self.side, self.rewards_hyperparam_dict) for i in
        ...
        if done: # TODO add solo?
            ...
            if (env.judge_red_win() == 1 and self.side == 0) or (env.judge_red_win() == -1 and self.side == 1):
                self.sampler_log["num_positive_reward_episodes"] = self.sampler_log[
                                                                       "num_positive_reward_episodes"] + 1
    ...
    def train(self, batchs):
        ...
        batch = dict(state_global=[], state_native=[],state_id=[],
                     ...
                     maneuver_masks=[], target_masks=[], shot_masks=[],
                     next_state_global=[], next_state_native=[], next_state_token=[],
                     team_rewards=[], solo_rewards=[], other_mean_reward=[])
        ...
        for epoch in range(Config.epochs):
            ...
            for i in range(optim_iter_num):
                ...
                maneuver_prob, shoot_prob, target_prob, _ = model.forward_with_mask(states_global_b, states_native_b,
                                                                                    states_token_b, self_msl_token_b,
