...
from models.independ_nn.independ_semantic_nn import Independ_Semantic_NN
from models.independ_nn.ltt_nn import Ltt_NN
...
class Independ_Ltt_Semantic_Agent(AgentBase):
    def __init__(self, target_type=None, agent_save_mode=None, reward_type=None, reward_update_type=None, reward_hyperparam_dict=None):
        self.batchs = dict(state_global=[], state_native=[], state_id=[],
                           ...
                           ver_one_hots=[], mask=[], ltt=[], hor_one_hots_ltt=[], ver_one_hots_ltt=[],
                           maneuver_one_hots_ltt=[],
        ...
        self.hor_one_hots_ltt = None
        self.ver_one_hots_ltt = None
        self.maneuver_one_hots_ltt = None
    ...
    def _create_model(self):
        ...
        model = []
        model_major = Independ_Semantic_NN(Config.env.red, len(red_state_global[0]), len(red_atten_state[0][0]),
        ...
        model_ltt = Ltt_NN(Config.env.red, len(red_state_global[0]), len(red_atten_state[0][0]),
                           len(red_atten_state[1][0][0]), len(red_atten_state[1][0]),
                           len(msl_token_self[0][0]), len(msl_token_self[0]),
                           len(msl_token_bandit[0][0]), len(msl_token_bandit[0]),
                           action_dims=self.model_action_dims)
        model.append(model_major)
        model.append(model_ltt)
        return model
    ...
    def before_step_for_sample(self, env):
        ...
        if self.target_type == "without_self":
            hor_list, ver_list, shots_list, targets_list, v_c_list, nn_c_list = model[0].select_action(
                self.states_global,
                self.states_native,
                self.states_tokens,
                msl_token_self,
        ...
        if self.step == 0:
            self.last_hor = deepcopy(hor_list.tolist())
            self.last_ver = deepcopy(ver_list.tolist())
            self.last_target = deepcopy(targets_list.tolist())
            self.last_v = deepcopy(v_c_list.tolist())
            self.last_nn = deepcopy(nn_c_list.tolist())

        last_hor_one_hots = [index_to_one_hot(self.last_hor[i], hor_mask_len) for i in range(self.team_aircraft_num)]
        self.hor_one_hots_ltt = deepcopy(last_hor_one_hots)
        last_hor_one_hots = torch.tensor(last_hor_one_hots).detach()

        last_ver_one_hots = [index_to_one_hot(self.last_ver[i], ver_mask_len) for i in range(self.team_aircraft_num)]
        self.ver_one_hots_ltt = deepcopy(last_ver_one_hots)
        last_ver_one_hots = torch.tensor(last_ver_one_hots).detach()

        maneuver_num = hor_mask_len * ver_mask_len
        maneuver_idx = [ver_mask_len * self.last_hor[i][0] + self.last_ver[i][0] for i in range(self.team_aircraft_num)]
        last_maneuver_one_hots = [index_to_one_hot(maneuver_idx[i], maneuver_num) for i in range(self.team_aircraft_num)]
        self.maneuver_one_hots_ltt = deepcopy(last_maneuver_one_hots)
        last_maneuver_one_hots = torch.tensor(last_maneuver_one_hots).detach()

        ltt_list = model[1].select_action(self.states_global,
                                          self.states_native,
                                          self.states_tokens,
        ...
        for i in range(self.side * env.red, env.red + self.side * env.blue):
            if not self.dones[i]:
                cur_ltt = ltt_list[i if i < env.red else i - env.red].tolist()[0]
                if cur_ltt or self.step % Config.ltt_interval == 0:
                    ...
                    self.last_hor[i if i < env.red else i - env.red][0] = cur_hor
                    self.last_ver[i if i < env.red else i - env.red][0] = cur_ver
                else:
                    cur_hor = self.last_hor[i if i < env.red else i - env.red][0]
                    cur_ver = self.last_ver[i if i < env.red else i - env.red][0]
                if self.step % Config.ltt_interval == 0:
                    ...
                    self.last_v[i if i < env.red else i - env.red][0] = cur_v
                    self.last_nn[i if i < env.red else i - env.red][0] = cur_nn
                    self.last_target[i if i < env.red else i - env.red][0] = cur_target
                else:
                    cur_v = self.last_v[i if i < env.red else i - env.red][0]
                    cur_nn = self.last_nn[i if i < env.red else i - env.red][0]
                    cur_target = self.last_target[i if i < env.red else i - env.red][0]
                    cur_shot = 0
        ...
        return hor_out, ver_out, shot_out, target_out, v_c_out, nn_c_out, ltt_out, hor_masks, ver_masks, shot_masks, target_masks
    ...
    def before_step_for_train(self, env):
        ...
        with torch.no_grad():
            ...
            hor_out, ver_out, shot_out, target_out, v_c_out, nn_c_out, ltt_out, hor_masks, ver_masks, \
            shot_masks, target_masks = self.before_step_for_sample(env)
            ...
            for i in range(self.side * env.red, env.red + self.side * env.blue):
                if self.dones[i]:
                    ...
                    self.ltts.append(ltt_out[i if i < env.red else i - env.red])

                if self.dones[i] or math.isnan(float(hor_out[i if i < env.red else i - env.red])):
                ...
                if self.dones[i] or math.isnan(float(ver_out[i if i < env.red else i - env.red])):
    ...
    def after_step_for_train(self, env):
        ...
        solo_sum_rewards = [
            reward_method_independ.get_ith_aircraft_reward(env, i, self.side, self.rewards_hyperparam_dict) for i in
        ...
        final_rewards = [
            ...
            Config.Independ.pro_factor_alpha * other_mean_reward[i] + Config.Independ.beta * team_rewards
        ...
        self.batchs["ltt"].append(self.ltts)
        self.batchs["hor_one_hots_ltt"].append(self.hor_one_hots_ltt)
        self.batchs["ver_one_hots_ltt"].append(self.ver_one_hots_ltt)
        self.batchs["maneuver_one_hots_ltt"].append(self.maneuver_one_hots_ltt)
    ...
    def train(self, batchs):
        ...
        batch = dict(state_global=[], state_native=[], state_id=[],
                     state_token=[], self_msl_token=[], bandit_msl_token=[],
                     hor=[], ver=[], shot=[], target=[], v_c=[], nn_c=[], ltt=[],
                     hor_one_hots=[], ver_one_hots=[], hor_one_hots_ltt=[], ver_one_hots_ltt=[],
                     maneuver_one_hots_ltt=[], mask=[],
                     hor_masks=[], ver_masks=[], target_masks=[], shot_masks=[], mask_solo_done=[],
                     next_state_global=[], next_state_native=[], next_state_token=[], team_rewards=[], solo_rewards=[],
                     shape_rewards=[], other_mean_reward=[], final_rewards=[])
        ...
        hor_one_hots_ltt = torch.tensor(batch["hor_one_hots_ltt"]).to(dtype).to(device)
        ver_one_hots_ltt = torch.tensor(batch["ver_one_hots_ltt"]).to(dtype).to(device)
        maneuver_one_hots_ltt = torch.tensor(batch["maneuver_one_hots_ltt"]).to(dtype).to(device)
        ...
        for epoch in range(Config.epochs):
            ...
            hor_one_hots_ltt = hor_one_hots_ltt[perm].clone()
            ver_one_hots_ltt = ver_one_hots_ltt[perm].clone()
            maneuver_one_hots_ltt = maneuver_one_hots_ltt[perm].clone()
            ...
            for i in range(optim_iter_num):
                ...
                hor_one_hots_b = hor_one_hots[ind]
                ver_one_hots_b = ver_one_hots[ind]
                hor_one_hots_ltt_b = hor_one_hots_ltt[ind]
                ver_one_hots_ltt_b = ver_one_hots_ltt[ind]
                maneuver_one_hots_ltt_b = maneuver_one_hots_ltt[ind]
                ...
                log_probs, values_pred = model[0].get_log_prob_and_values(states_global_b, states_native_b,
                                                                          ...
                                                                          ver_one_hots_b,
                                                                          v_cs_b, nn_cs_b)
                ltt_probs, _ = model[1].get_log_prob_and_values(states_global_b, states_native_b,
                                                                states_token_b,
                                                                self_msl_token_b,
                                                                bandit_msl_token_b, states_id_b,
                                                                ltts_b,
                                                                hor_one_hots_ltt_b,
                                                                ver_one_hots_ltt_b,
                                                                maneuver_one_hots_ltt_b,
                                                                False)
                ...
                # ltt loss #
                # value_ltt_loss = (values_pred_ltt - returns_ltt_b).pow(2).mean()
                # # weight decay
                # for param in model[1].parameters():
                #     value_ltt_loss += param.pow(2).sum() * 1e-3
                ltt_loss = (ltt_probs.squeeze(-1) * (advantages_b + Config.delib_cost)).mean()  # sigmoid
                # policy_ltt_loss = -1 * dual_ppo_loss(log_probs_ltt, fixed_log_probs_ltt_b, advantages_b + Config.delib_cost) # sigmoid
                # policy_ltt_loss = dual_ppo_loss(log_probs_ltt, fixed_log_probs_ltt_b, advantages_ltt_b) # softmax

                # loss_ltt = policy_ltt_loss + 1e-3 * value_ltt_loss
                loss_ltt = ltt_loss

                loss_step(model[0], loss_major)
                loss_step(model[1], loss_ltt)
