...
from models.pomdp_nn.base_nn import Base_NN

# from state_method import state_method_independ, state_method_independ_lite
from state_method import state_method_pomdp, state_method_independ
...
class Base_agent(AgentBase):
    def __init__(self, target_type=None, agent_save_mode=None, reward_type=None, reward_update_type=None,
                 reward_hyperparam_dict=None, seq_len=5, is_lstm=False, DDP=False, bandit_predict_len=10):
        self.batchs = dict(state_global=[], state_native=[], state_id=[],
                           ...
                           shape_rewards=[],
                           solo_rewards=[], other_mean_reward=[], final_rewards=[],
                           bandit_predict_state=[], bandit_predict_label=[],
                           log=None)
        ...
        # for pomdp
        self.bandit_predict_state = None    # 2 * 6 : [x, y, z, vx, vy, vz]
        self.bandit_predict_label = None
        self.bandit_predict_len = seq_len    # todo maybe longer and independ of policy lstm

    def _create_model(self):
        state = state_method_pomdp.get_kteam_aircraft_state(Config.env, 0)
        red_state_global = state[0]
        red_state_native = state[1]
        red_state_atten = state[2]
        msl_token_self = state[3]
        msl_token_bandit = state[4]
        bandit_predict_state = state[6]
        ...
        self.model_action_dims = dict(
            ...
            bandit_predict_out_dim=Config.env.action_interface["red_awacs"][0]
        )
        ...
        self.state_dims = dict(
            ...
            bandit_predict_state_dim=len(bandit_predict_state[0][0]),    # bandit predict feature num
            bandit_predict_num=len(bandit_predict_state[0])       # bandit num
        )
        ...
        model = Base_NN(self.state_dims['id_dim'],
                                           ...
                                           self.state_dims['bandit_predict_state_dim'], self.state_dims['bandit_predict_num'],
    ...
    def after_reset(self, env, side):
        ...
        if side == "red":
            ...
            for i in range(env.red, env.red + env.blue):
                self.init_bandit_die += 1 - env.state_interface["AMS"][i]["alive"]["value"]
                # for pomdp --- Truth
                # bandit_predict.append([env.normalize(env.state_interface["AMS"][i]["Xg_0"]),
                #                        env.normalize(env.state_interface["AMS"][i]["Xg_1"]),
                #                        env.normalize(env.state_interface["AMS"][i]["Xg_2"]),
                #                        env.normalize(env.state_interface["AMS"][i]["Vg_0"]),
                #                        env.normalize(env.state_interface["AMS"][i]["Vg_1"]),
                #                        env.normalize(env.state_interface["AMS"][i]["Vg_2"]),
                #                        1.0
                #                        ])
                # for pomdp --- Predict
                bandit_predict.append([env.normalize(env.state_interface["AMS"][i]["Xg_0_est"]),
                                       env.normalize(env.state_interface["AMS"][i]["Xg_1_est"]),
                                       env.normalize(env.state_interface["AMS"][i]["Xg_2_est"]),
                                       env.normalize(env.state_interface["AMS"][i]["Vg_0_est"]),
                                       env.normalize(env.state_interface["AMS"][i]["Vg_1_est"]),
                                       env.normalize(env.state_interface["AMS"][i]["Vg_2_est"]),
                                       0.5
                                       ])
        elif side == "blue":
            self.side = 1
        ...
        # for pomdp
        self.bandit_predict_state = [[bandit_predict] * self.bandit_predict_len] * self.team_aircraft_num
    ...
    def before_step_for_sample(self, env):
        ...
        state = state_method_pomdp.get_kteam_aircraft_state(Config.env, self.side)
        ...
        for i in range(self.team_aircraft_num):
            ...
            # for pomdp
            bandit_state = deepcopy(self.bandit_predict_state[i])
            bandit_state.remove(bandit_state[0])
            self.bandit_predict_state[i] = bandit_state
            self.bandit_predict_state[i].append(bandit_predict_state[i])
        ...
        if self.target_type == "without_self":
            hor_list, ver_list, shots_list, targets_list, v_c_list, nn_c_list, lts_list, new_hiddens, bandit_predict = \
                model.select_action(self.states_global, self.states_native, self.states_tokens,
                                    self.self_msl_tokens, self.states_id,
        ...
        # for pomdp 根据神经网络预测输出，返回预警机信息
        # bandit_predict_ith 神经网络输出[-1, 1]
        # bandit_predict_res 反归一化为坐标和速度的实值，返回接口
        # bandit_label 归一化为[-1， 1]， 用于与bandit_predict_ith作 MSE_loss
        # 此处（反）归一化范围人为干预， 范围相同

        bandit_predict_list = []
        bandit_label_list = []
        if (not self.side) and "red_awacs" in env.action_interface.keys():
            for i in range(env.blue):
                bandit_predict_ith = bandit_predict[i].squeeze(0).tolist()
                bandit_predict_res, bandit_label = state_method_pomdp.bandit_predict_process(env, bandit_predict_ith, i+env.red)
                env.action_interface["red_awacs"][i]["action_xg_0_est"]["value"] = bandit_predict_res[0]
                env.action_interface["red_awacs"][i]["action_xg_1_est"]["value"] = bandit_predict_res[1]
                env.action_interface["red_awacs"][i]["action_xg_2_est"]["value"] = bandit_predict_res[2]
                env.action_interface["red_awacs"][i]["action_vg_0_est"]["value"] = bandit_predict_res[3]
                env.action_interface["red_awacs"][i]["action_vg_1_est"]["value"] = bandit_predict_res[4]
                env.action_interface["red_awacs"][i]["action_vg_2_est"]["value"] = bandit_predict_res[5]
                bandit_predict_list.append(bandit_predict_res)
                bandit_label_list.append(bandit_label)

        if self.side and "blue_awacs" in env.action_interface.keys():
            for i in range(env.red):
                bandit_predict_ith = bandit_predict[i].squeeze(0).tolist()
                bandit_predict_res, bandit_label = state_method_pomdp.bandit_predict_process(env, bandit_predict_ith, i)
                env.action_interface["blue_awacs"][i]["action_xg_0_est"]["value"] = bandit_predict_res[0]
                env.action_interface["blue_awacs"][i]["action_xg_1_est"]["value"] = bandit_predict_res[1]
                env.action_interface["blue_awacs"][i]["action_xg_2_est"]["value"] = bandit_predict_res[2]
                env.action_interface["blue_awacs"][i]["action_vg_0_est"]["value"] = bandit_predict_res[3]
                env.action_interface["blue_awacs"][i]["action_vg_1_est"]["value"] = bandit_predict_res[4]
                env.action_interface["blue_awacs"][i]["action_vg_2_est"]["value"] = bandit_predict_res[5]
                bandit_predict_list.append(bandit_predict_res)
                bandit_label_list.append(bandit_label)
        ...
        return hor_out, ver_out, shot_out, target_out, v_c_out, nn_c_out, hor_masks, ver_masks, shot_masks, target_masks, lts_out, lts_masks, bandit_label_list
    ...
    def before_step_for_train(self, env):
        ...
        with torch.no_grad():
            ...
            hor_out, ver_out, shot_out, target_out, v_c_out, nn_c_out, hor_masks, ver_masks, shot_masks, target_masks, lts_out, lts_masks, bandit_label_list = \
            ...
            # for pomdp
            self.bandit_predict_label = bandit_label_list
    ...
    def after_step_for_train(self, env):
        ...
        # for pomdp
        self.batchs["bandit_predict_state"].append(deepcopy(self.bandit_predict_state))
        self.batchs["bandit_predict_label"].append(deepcopy(self.bandit_predict_label))
    ...
    def ddp_parallel_train(self, batchs):
        ...
        count = torch.cuddenormalizea.device_count()
    ...
    def single_gpu_train(self, batchs, model_optimizer, gpu_rank=0, world_size=1):
        ...
        batch = dict(state_global=[], state_native=[], state_id=[],
                     state_token=[], self_msl_token=[], last_step_decision=[],
                     deliberation_cost=[],
        ...
        # for pomdp
        bandit_predict_state = torch.from_numpy(np.array(batch["bandit_predict_state"])).to(dtype).to(device)
        bandit_predict_label = torch.from_numpy(np.array(batch["bandit_predict_label"])).to(dtype).to(device)
        ...
        with torch.no_grad():
            for i in range(batch_forward_times):
                ...
                # for pomdp
                bandit_predict_state_b = bandit_predict_state[ind]
                ...
                v_cs_b = v_cs[ind]
                nn_cs_b = nn_cs[ind]
                fixed_log_probs_b, values_b, _= self.model.get_log_prob_and_values(states_global_b, states_native_b,
                                                                                 states_token_b,
                                                                                 self_msl_token_b,
                                                                                 states_id_b,
        ...
        for epoch in range(Config.epochs):
            ...
            # for pomdp
            bandit_predict_state = bandit_predict_state[perm].clone()
            bandit_predict_label = bandit_predict_label[perm].clone()
            ...
            for i in range(optim_iter_num):
                ...
                # for pomdp
                bandit_predict_state_b = bandit_predict_state[ind]
                bandit_predict_label_b = bandit_predict_label[ind]
                ...
                log_probs, values_pred, ret_for_loss = self.model.get_log_prob_and_values(states_global_b, states_native_b,
                                                                       states_token_b,
                                                                       self_msl_token_b,
                                                                       states_id_b,
                                                                       hors_b, vers_b, shots_b, targets_b,
                ...
                #* calculate loss *#
                hor_prob, ver_prob, shoot_prob, target_prob, v_c_prob, nn_c_prob, value, lts_prob, bandit_predict_0, bandit_predict_1 = \
                ret_for_loss[0], ret_for_loss[1], ret_for_loss[2], ret_for_loss[3], ret_for_loss[4], ret_for_loss[5], \
                ret_for_loss[6], ret_for_loss[7], ret_for_loss[8], ret_for_loss[9]


                # 1. supervise loss for bandit predict #
                # label : bandit_predict_label_b   (batch * bandit_num * feature_num)
                # out : bandit_predict_0, bandit_predict_1 (batch * feature_num)
                bandit_predict_out_b = torch.stack([bandit_predict_0, bandit_predict_1], dim=1)
                # bandit_predict_loss = ((bandit_predict_out_b - bandit_predict_label_b) * torch.repeat_interleave(
                #     current_step_available_b.unsqueeze(-1), repeats=6, dim=-1)).pow(2).mean()

                # todo dim 6 or 3
                bandit_predict_x_loss = ((bandit_predict_out_b[:, :, :3] - bandit_predict_label_b[:, :, :3]) * torch.repeat_interleave(
                    current_step_available_b.unsqueeze(-1), repeats=3, dim=-1)).pow(2).mean()

                bandit_predict_v_loss = ((bandit_predict_out_b[:, :, 3:] - bandit_predict_label_b[:, :, 3:]) * torch.repeat_interleave(
                    current_step_available_b.unsqueeze(-1), repeats=3, dim=-1)).pow(2).mean()
                ...
                # 4. calculate policy entropy loss
                ...
                #     hidden_h_b.unsqueeze(0), hidden_c_b.unsqueeze(0), bandit_predict_state_b)
                ...
                loss = policy_loss \
                       ...
                       - 0.01 * lts_entropy_loss \
                       + bandit_predict_x_loss \
                       + bandit_predict_v_loss

                # print(policy_loss, bandit_predict_x_loss, bandit_predict_v_loss)
...
if __name__ == "__main__":
    ...
    red_agent = Base_agent(target_type="without_self", agent_save_mode="origin",
                              reward_type="expert_rewards", is_lstm=True,
                              reward_hyperparam_dict=origin_reward_parameters, DDP=False)
    blue_agent = Base_agent(target_type="without_self", agent_save_mode="origin",
                               reward_type="expert_rewards", is_lstm=True,
                               reward_hyperparam_dict=origin_reward_parameters, DDP=False)
