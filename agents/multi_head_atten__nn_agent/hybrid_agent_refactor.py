...
# 2020/09/12 refactor from origin hybrid agent #

from framwork.agent_base import AgentBase
# from models.hybrid_nn import Hybrid_NN
...
from state_method import state_method_refactor
...
class Hybrid_Agent(AgentBase):
    def __init__(self, target_type=None, agent_save_mode=None, reward_type=None, reward_update_type=None, reward_hyperparam_dict=None):
        self.batchs = dict(state_global=[], state_native=[],
                           ...
                           maneuver=[], shot=[], target=[], v_c=[], nn_c=[],
                           maneuver_one_hots=[], target_one_hots=[], shoot_one_hots=[],
        ...
        self.target_one_hots = None
        self.shoot_one_hots = None
    ...
    def before_step_for_sample(self, env):
        ...
        if self.target_type == "origin" or self.target_type == "without_self":
            maneuver_list, shots_list, targets_list, v_c_list, nn_c_list = model.select_action(self.states_global, self.states_native,
                                                                                               self.states_tokens, self.self_msl_tokens,
                                                                                               self.bandit_msk_tokens,
                                                                                               target_masks, maneuver_masks, shot_masks)
        ...
        for i in range(self.side * env.red, env.red + self.side * env.blue):
            if not self.dones[i]:
                ...
                if rand_num < 0:  # todo, magic number here  # 0 for close this item
                    ...
                    env.action_interface["AMS"][i]["HybridManeuver"]["action_side"]["value"] = 0

                    env.action_interface["AMS"][i]["HybridManeuver"]["action_afterburning"]["value"] = 0
        ...
        with torch.no_grad():
            ...
            for i in range(self.side * env.red, env.red + self.side * env.blue):
                if self.dones[i]:
                    ...
                    self.shoot_one_hots.append([0] * self.model_action_dims["shoot_dim"])
                    self.target_one_hots.append([0] * self.model_action_dims["target_dim"])
                else:
                    ...
                    self.shoot_one_hots.append(index_to_one_hot(shot_out[i if i < env.red else i - env.red],
                                                                self.model_action_dims["shoot_dim"]))
                    self.target_one_hots.append(index_to_one_hot(target_out[i if i < env.red else i - env.red],
                                                                 self.model_action_dims["target_dim"]))
    ...
    def after_step_for_train(self, env):
        ...
        self.batchs["target_one_hots"].append(self.target_one_hots)
        self.batchs["shoot_one_hots"].append(self.shoot_one_hots)
    ...
    def train(self, batchs):
        ...
        shoot_one_hots = torch.tensor(batch["shoot_one_hots"]).to(dtype).to(device)
        target_one_hots = torch.tensor(batch["target_one_hots"]).to(dtype).to(device)
        ...
        for epoch in range(Config.epochs):
            ...
            shoot_one_hots = shoot_one_hots[perm].clone()
            target_one_hots = target_one_hots[perm].clone()
            ...
            for i in range(optim_iter_num):
                ...
                shoot_one_hots_b = shoot_one_hots[ind]
                target_one_hots_b = target_one_hots[ind]
                ...
                log_probs, values_pred = model.get_log_prob_and_values(states_global_b, states_native_b, states_token_b,
                                                                       ...
                                                                       target_one_hots_b, maneuver_one_hots_b,
                                                                       shoot_one_hots_b,
                                                                       v_cs_b, nn_cs_b)
