...
# from state_method import state_method_independ, state_method_independ_lite
from state_method import state_method_independ_refactor, state_method_independ
...
class Release_Agent(AgentBase):
    def __init__(self, target_type=None, agent_save_mode=None, reward_type=None, reward_update_type=None,
        ...
        self.batch_states_global = None
        self.batch_states_native = None
        self.batch_states_id = None
        self.batch_states_token = None
        self.batch_bandit_msl_token = None
        self.batch_self_msl_token = None
        self.batch_last_step_decision = None
        self.batch_deliberation_cost = None
        self.batch_hors = None
        self.batch_vers = None
        self.batch_shots = None
        self.batch_final_rewards = None
        self.batch_masks = None
        self.batch_mask_solo_done = None
        self.batch_current_step_available = None
        self.batch_targets = None
        self.batch_v_cs = None
        self.batch_nn_cs = None
        self.batch_hor_one_hots = None
        self.batch_ver_one_hots = None
        self.batch_lts = None
        self.deliberation_cost_batch = None

        self.batch_hor_masks = None
        self.batch_ver_masks = None
        self.batch_target_masks = None
        self.batch_shot_masks = None
        self.batch_lts_masks = None
        self.batch_hidden_h = None
        self.batch_hidden_c = None

        # for tensorboardX logging #
        self.param_dict = None
        self.param_grad_dict = None
    ...
    def _create_model(self):
        ...
        self.state_dims = dict(
            ...
            native_dim=len(red_state_native[0]),
            state_token_dim=len(red_state_atten[0][0]),
            state_token_num=len(red_state_atten[0]),
    ...
    def before_step_for_sample(self, env):
        ...
        state = state_method_independ_refactor.get_kteam_aircraft_state(Config.env, self.side)

        state_id_one_hot = state[5]
        states_global = state[0]
        states_atten = [state[1], state[2]]
        msl_token_self = state[3]
        msl_token_bandit = state[4]
    ...
    def after_step_for_train(self, env):
        ...
        shape_reward = [reward_shaping.get_ith_aircraft_shaped_reward(env, i, self.side, self.interval, self_hor=self.hors, last_hor=self.last_hors,
                                                                      self_ver=self.vers, last_ver=self.last_vers,
                                                                      self_target=self.targets, last_target=self.last_targets,
                                                                      rewards_hyperparam_dict=self.rewards_hyperparam_dict) for i in range(self.side * env.red, env.red + self.side * env.blue)]
        ...
        self.batchs["hidden_h"].append(deepcopy([self.hidden_h[0][0], self.hidden_h[1][0]]))
        self.batchs["hidden_c"].append(deepcopy([self.hidden_c[0][0], self.hidden_c[1][0]]))
    ...
    def ddp_parallel_train(self, batchs):
        ...
        if world_size == 0:
            # error message
            return
        # change training data from list to tensor
        batch_len = self.decode(batchs)
        for rank in range(world_size):
            learners.append(smp.Process(target=self.parallel_train, args=(world_size, rank, batch_len)))
    ...
    def decode(self, batchs):
        ...
        self.batch_states_global = torch.from_numpy(np.array(batch["state_global"])).to(dtype).to(device)
        self.batch_states_native = torch.from_numpy(np.array(batch["state_native"])).to(dtype).to(device)
        self.batch_states_id = torch.from_numpy(np.array(batch["state_id"])).to(dtype).to(device)
        self.batch_states_token = torch.from_numpy(np.array(batch["state_token"])).to(dtype).to(device)
        self.batch_self_msl_token = torch.from_numpy(np.array(batch["self_msl_token"])).to(dtype).to(device)
        self.batch_bandit_msl_token = torch.from_numpy(np.array(batch["bandit_msl_token"])).to(dtype).to(device)
        self.batch_last_step_decision = torch.from_numpy(np.array(batch["last_step_decision"])).to(dtype).to(device)
        self.batch_deliberation_cost = torch.from_numpy(np.array(batch["deliberation_cost"])).to(dtype).to(device)

        self.batch_hors = torch.from_numpy(np.array(batch["hor"])).to(dtype).to(device)
        self.batch_vers = torch.from_numpy(np.array(batch["ver"])).to(dtype).to(device)
        self.batch_shots = torch.from_numpy(np.array(batch["shot"])).to(dtype).to(device)
        self.batch_final_rewards = torch.from_numpy(np.array(batch["final_rewards"])).to(dtype).to(device)
        self.batch_masks = torch.from_numpy(np.array(batch["mask"])).to(dtype).to(device)
        self.batch_mask_solo_done = torch.from_numpy(np.array(batch["mask_solo_done"])).to(dtype).to(device)
        self.batch_current_step_available = torch.from_numpy(np.array(batch["current_step_available"])).to(dtype).to(device)
        self.batch_targets = torch.from_numpy(np.array(batch["target"])).to(dtype).to(device)
        self.batch_v_cs = torch.from_numpy(np.array(batch["v_c"])).to(dtype).to(device)
        self.batch_nn_cs = torch.from_numpy(np.array(batch["nn_c"])).to(dtype).to(device)
        self.batch_hor_one_hots = torch.tensor(batch["hor_one_hots"]).to(dtype).to(device)
        self.batch_ver_one_hots = torch.tensor(batch["ver_one_hots"]).to(dtype).to(device)
        self.batch_lts = torch.tensor(batch["lts"]).to(dtype).to(device)

        self.batch_hor_masks = torch.from_numpy(np.array(batch["hor_masks"])).to(dtype).to(device)
        self.batch_ver_masks = torch.from_numpy(np.array(batch["ver_masks"])).to(dtype).to(device)
        self.batch_target_masks = torch.from_numpy(np.array(batch["target_masks"])).to(dtype).to(device)
        self.batch_shot_masks = torch.from_numpy(np.array(batch["shot_masks"])).to(dtype).to(device)
        self.batch_lts_masks = torch.from_numpy(np.array(batch["lts_masks"])).to(dtype).to(device)

        self.batch_hidden_h = torch.from_numpy(np.array(batch["hidden_h"])).to(dtype).contiguous().to(device)
        self.batch_hidden_c = torch.from_numpy(np.array(batch["hidden_c"])).to(dtype).contiguous().to(device)
        batch_len = self.batch_states_native.shape[0]
        return batch_len
    ...
    def parallel_train(self, world_size, rank, batch_len):
        os.environ[
            'MASTER_ADDR'] = Config.local_address  # tell Multiprocessing model which IP of process to synchronize model
        ...
        self.mutil_gpu_train(model_optimizer, rank, world_size, batch_len)
        if rank == 0:
    ...
    def mutil_gpu_train(self, model_optimizer, gpu_rank, world_size, batch_len):
        device = torch.device("cuda", gpu_rank)
        # the num of training data for one gpu
        each_gpu = int(batch_len/world_size)
        states_global = self.batch_states_global[gpu_rank * each_gpu:(gpu_rank+1)*each_gpu].to(device)
        states_native = self.batch_states_native[gpu_rank * each_gpu:(gpu_rank+1)*each_gpu].to(device)
        states_id = self.batch_states_id[gpu_rank * each_gpu:(gpu_rank+1)*each_gpu].to(device)
        states_token = self.batch_states_token[gpu_rank * each_gpu:(gpu_rank+1)*each_gpu].to(device)
        self_msl_token = self.batch_self_msl_token[gpu_rank * each_gpu:(gpu_rank+1)*each_gpu].to(device)
        bandit_msl_token = self.batch_bandit_msl_token[gpu_rank * each_gpu:(gpu_rank+1)*each_gpu].to(device)
        last_step_decision = self.batch_last_step_decision[gpu_rank * each_gpu:(gpu_rank+1)*each_gpu].to(device)
        deliberation_cost = self.batch_deliberation_cost[gpu_rank * each_gpu:(gpu_rank+1)*each_gpu].to(device)

        hors = self.batch_hors[gpu_rank * each_gpu:(gpu_rank+1)*each_gpu].to(device)
        vers = self.batch_vers[gpu_rank * each_gpu:(gpu_rank+1)*each_gpu].to(device)
        shots = self.batch_shots[gpu_rank * each_gpu:(gpu_rank+1)*each_gpu].to(device)
        ...
        final_rewards = self.batch_final_rewards[gpu_rank * each_gpu:(gpu_rank+1)*each_gpu].to(device)
        masks = self.batch_masks[gpu_rank * each_gpu:(gpu_rank+1)*each_gpu].to(device)
        mask_solo_done = self.batch_mask_solo_done[gpu_rank * each_gpu:(gpu_rank+1)*each_gpu].to(device)
        current_step_available = self.batch_current_step_available[gpu_rank * each_gpu:(gpu_rank+1)*each_gpu].to(device)
        targets = self.batch_targets[gpu_rank * each_gpu:(gpu_rank+1)*each_gpu].to(device)
        v_cs = self.batch_v_cs[gpu_rank * each_gpu:(gpu_rank+1)*each_gpu].to(device)
        nn_cs = self.batch_nn_cs[gpu_rank * each_gpu:(gpu_rank+1)*each_gpu].to(device)
        hor_one_hots = self.batch_hor_one_hots[gpu_rank * each_gpu:(gpu_rank+1)*each_gpu].to(device)
        ver_one_hots = self.batch_ver_one_hots[gpu_rank * each_gpu:(gpu_rank+1)*each_gpu].to(device)
        lts = self.batch_lts[gpu_rank * each_gpu:(gpu_rank+1)*each_gpu].to(device)

        hor_masks = self.batch_hor_masks[gpu_rank * each_gpu:(gpu_rank+1)*each_gpu].to(device)
        ver_masks = self.batch_ver_masks[gpu_rank * each_gpu:(gpu_rank+1)*each_gpu].to(device)
        target_masks = self.batch_target_masks[gpu_rank * each_gpu:(gpu_rank+1)*each_gpu].to(device)
        shot_masks = self.batch_shot_masks[gpu_rank * each_gpu:(gpu_rank+1)*each_gpu].to(device)
        lts_masks = self.batch_lts_masks[gpu_rank * each_gpu:(gpu_rank+1)*each_gpu].to(device)

        hidden_h = self.batch_hidden_h[gpu_rank * each_gpu:(gpu_rank+1)*each_gpu].to(device).contiguous()
        hidden_c = self.batch_hidden_c[gpu_rank * each_gpu:(gpu_rank+1)*each_gpu].to(device).contiguous()
        ...
        optim_iter_num = int(Config.mini_batch_size / Config.optim_batch_size)
    ...
    def single_gpu_train(self, batchs, model_optimizer, gpu_rank=0, world_size=1):
        ...
        for epoch in range(Config.epochs):
            ...
            for i in range(optim_iter_num):
                ...
                if torch.isnan(loss):
                    ...
                    loss.backward()
                    if self.param_grad_dict is None:
                        self.param_grad_dict = dict()
                        for i, (name, param) in enumerate(self.model.named_parameters()):
                            if param.grad is None:
                                pass
                            else:
                                self.param_grad_dict[name + "_grad"] = param.data.tolist()
                                # print(self.param_grad_dict)
                    else:
                        pass
                    ...
                    for param in self.model.parameters():
                        ...
                        # if True:
                            f_grad = open("grad", "w")
                            json.dump(self.param_grad_dict, f_grad)
                            f_grad.close()
                            break
        ...
        for i, (name, param) in enumerate(self.model.named_parameters()):
            self.param_dict[name + "_data"] = param.data.tolist()
