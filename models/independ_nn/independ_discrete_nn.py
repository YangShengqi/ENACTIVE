...
class Independ_Discrete_NN(nn.Module):
    ...
    def forward_with_mask(self, global_state, native_state, token_state, self_msl_token_state,
                          bandit_msl_token_state, ids_one_hot_state,
    ...
    def forward(self, global_state, native_state, token_state, self_msl_token_state, bandit_msl_token_state):
        ...
        maneuver_probs = torch.softmax(self.maneuver_heads(m_hiddens), dim=-1)
    ...
    def select_action(self, x0, x1, x2, x3, x4, x5, maneuver_masks: list, shoot_masks: list, target_masks: list, mask_forward=True):
        ...
        if mask_forward:
            maneuver_probs, shoot_probs, target_probs, _ = self.forward_with_mask(x0, x1, x2, x3, x4, x5,
    ...
    def get_log_prob_and_values(self, x0, x1, x2, x3, x4, x5, maneuvers, shoots, targets, maneuver_masks, shoot_masks, target_masks, mask_forward=True):

        if mask_forward:
            maneuver_probs, shoot_probs, target_probs, value = self.forward_with_mask(x0, x1, x2, x3, x4, x5,
                                                                                      target_masks, maneuver_masks,
                                                                                      shoot_masks)
        else:
