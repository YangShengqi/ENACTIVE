...
class Independ_Semantic_NN(nn.Module):
    def __init__(self, id_dim, ground_truth_dim, native_dim, state_token_dim, state_token_num,
        ...
        self.target_hiddens = nn.Linear(last_policy_dim + state_token_num * state_token_num, action_head_hidden_size)
    ...
    def macro_forward(self, global_state, native_state, token_state, self_msl_token_state, bandit_msk_token_state,
        ...
        target_weight_list = []  # todo add weight to target list
        ...
        for air_id in range(self.aircraft_num):
            ...
            for i in range(self.attn_depth):
                ...
                state_tokens_out, target_weight = self.state_attn_layers[i](q_state, k_state, v_state)
            ...
            target_weight_flat_list = target_weight.flatten(start_dim=-2, end_dim=-1)  # todo add weight to target list
            target_weight_list.append(target_weight_flat_list.detach())
        ...
        target_weight_flat = torch.stack(target_weight_list, dim=1).squeeze(0)  # todo add weight to target list
        ...
        t_hiddens = self.activation(self.target_hiddens(torch.cat([policy_state, target_weight_flat.squeeze(0)], dim=-1)))
