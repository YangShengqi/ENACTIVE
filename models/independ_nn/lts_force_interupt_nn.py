...
class LTS_Force_Interupt_NN(nn.Module):
    def __init__(self, id_dim, ground_truth_dim, native_dim, state_token_dim, state_token_num,
                 ...
                 ground_truth_size_before_cat=(256, 128),
        ...
        self.v_c_hiddens = nn.Linear(last_policy_dim + action_dims["horizontal_cmd_dim"] + action_dims["vertical_cmd_dim"], action_head_hidden_size)
        self.nn_c_hiddens = nn.Linear(last_policy_dim + action_dims["horizontal_cmd_dim"] + action_dims["vertical_cmd_dim"], action_head_hidden_size)
    ...
    def macro_forward(self, global_state, native_state, token_state, self_msl_token_state, bandit_msk_token_state,
                      ids_one_hot_state, hor_mask, ver_mask, shoot_mask, target_mask, last_decision, lts_mask, deliberation_cost):
        ...
        for air_id in range(self.aircraft_num):
            ...
            bandit_msk_token_state = bandit_msk_token_state_split[air_id].squeeze(1).transpose(0, 1)
        ...
        # print(policy_state.size(), last_target.size(), last_hor.size(), last_ver.size())
        ...
        return hor_probs, ver_probs, shoot_probs, target_probs, v_head, policy_state, lts_probs
    ...
    def select_action(self, global_state, native_state, token_state, self_msl_token_state, bandit_msk_token_state,
                      ids_one_hot_state, hor_masks: list, ver_masks: list, shoot_masks: list, target_masks: list, last_decision, lts_masks, deliberation_cost):
        ...
        hor_probs, ver_probs, shoot_probs, target_probs, _, policy_state, lts_probs = self.macro_forward(global_state,
                                                                                                         native_state,
                                                                                                         token_state,
                                                                                                         ...
                                                                                                         ver_masks,
                                                                                                         shoot_masks,
                                                                                                         target_masks, last_decision, lts_masks, deliberation_cost)
        ...
        return hors, vers, shoots, targets, v_c, nn_c, lts
    ...
    def batch_forward(self, global_state, native_state, token_state, self_msl_token_state, bandit_msk_token_state,
        ...
        hor_probs, ver_probs, shoot_probs, target_probs, v_head, policy_state, lts_prob = self.macro_forward(global_state,
                                                                                                   native_state,
                                                                                                   token_state,
                                                                                                   self_msl_token_state,
                                                                                                   bandit_msk_token_state,
    ...
    def get_log_prob_and_values(self, x0, x1, x2, x3, x4, x5, hors, vers, shoots, targets, hor_masks, ver_masks,
                                shoot_masks, target_masks, hor_one_hots, ver_one_hots, v_cs, nn_cs, last_decision, lts_masks, lts_choice, deliberation_cost):

        hor_probs, ver_probs, shoot_probs, target_probs, v_c_probs, nn_c_probs, value, lts_probs = self.batch_forward(
            x0, x1, x2, x3, x4, x5, hor_masks, ver_masks, shoot_masks, target_masks, hor_one_hots, ver_one_hots, last_decision, lts_masks, deliberation_cost)
    ...
    def distillation_forward(self, global_state,
                             native_state,
                             token_state,
                             self_msl_token_state,
        ...
        token_state_split = token_state.split(1, dim=1)
        self_msl_token_state_split = self_msl_token_state.split(1, dim=1)
        bandit_msk_token_state_split = bandit_msl_token_state.split(1, dim=1)
        ...
        policy_state = torch.cat(
            [ids_one_hot_state, native_state, state_tokens_out_flat, self_msl_tokens_out_flat,
             bandit_msl_tokens_out_flat], dim=-1)
        ...
        shoot_probs = torch.softmax(self.shoot_heads(s_hiddens), dim=-1)
        target_probs = torch.softmax(self.target_heads(t_hiddens), dim=-1)
        hor_probs = torch.softmax(self.horizontal_cmd_heads(hor_hiddens), dim=-1)
        ver_probs = torch.softmax(self.vertical_cmd_heads(ver_hiddens), dim=-1)
        ...
        return v_head, hor_probs, ver_probs, shoot_probs, target_probs, v_c_probs, nn_c_probs


if __name__ == "__main__":
    from state_method.lts_state_method import get_kteam_global_ground_truth_state, get_kteam_aircraft_state_for_attention, get_self_kteam_msl_tokens, get_bandit_kteam_msl_tokens, get_kteam_ids_one_hot_state
    ...
    hors, vers, shoots, targets, v_c, nn_c, lts = model.select_action(red_state_global, native_state, token_state, self_msl_token_state, bandit_msk_token_state,
                                                                      state_id_one_hot, hor_masks, ver_masks, shoot_masks, target_masks, last_decision, lts_mask, [[0, 0.4], [0, 0.4]])
