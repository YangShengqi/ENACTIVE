...
class LSTM_noMSL_NN(nn.Module):
    def __init__(self, id_dim, ground_truth_dim, native_dim, state_token_dim, state_token_num,
        ...
        #  ---------------------------------actor -----------------------------------------------------#
        flatten_dim_policy = id_dim + last_native_dim + self.state_token_embed_dim * state_token_num + \
                                     self.msl_token_embed_dim * self_msl_token_num

        self.policy_commnet = CommNetWork(flatten_dim_policy, policy_hidden_size, n_agents=self.aircraft_num,
        ...
        # ---------------------------------------------critic --------------------------------------------- #
        flatten_dim_critic = id_dim + last_global_dim + self.msl_token_embed_dim * bandit_msl_token_num + last_policy_dim
        self.value_affine_layers = nn.ModuleList()
        last_value_dim = flatten_dim_critic
    ...
    def macro_forward(self, global_state, native_state, token_state, self_msl_token_state, bandit_msk_token_state,
                      ids_one_hot_state, hor_mask, ver_mask, shoot_mask, target_mask, hidden_h, hidden_c):
        ...
        for air_id in range(self.aircraft_num):
            ...
            self_msl_token_state = self_msl_token_state.contiguous().view(self_msl_token_state.shape[0], -1, self_msl_token_state.shape[-1])
            bandit_msk_token_state = bandit_msk_token_state.contiguous().view(bandit_msk_token_state.shape[0], -1, bandit_msk_token_state.shape[-1])
            ...
            for i in range(self.attn_depth):
                ...
                self_msl_tokens_out_flat = self_msl_tokens_out_flat.view(batch_size, seq_len, self_msl_tokens_out_flat.shape[-1])
                bandit_msl_tokens_out_flat = bandit_msl_tokens_out_flat.view(batch_size, seq_len, bandit_msl_tokens_out_flat.shape[-1])
        ...
        policy_state = torch.cat(
            [ids_one_hot_state, native_state, state_tokens_out_flat, self_msl_tokens_out_flat], dim=-1)    # 1 2 4 482

        comm_state = self.policy_commnet(policy_state)

        policy_state = comm_state.view(-1, seq_len, comm_state.shape[-1])
        policy_state, next_hiddens = self.lstm_layer(policy_state, (hidden_h, hidden_c))
        policy_state = policy_state[:, -1, :].view(batch_size, self.aircraft_num, -1)
        ...
        value_state = torch.cat(
            [ids_one_hot_state[:, :, -1, :], global_state, bandit_msl_tokens_out_flat[:, :, -1, :], policy_state],
            dim=-1)
        ...
        return hor_probs, ver_probs, shoot_probs, target_probs, v_head, policy_state.squeeze(0), next_hiddens
    ...
    def select_action(self, global_state, native_state, token_state, self_msl_token_state, bandit_msk_token_state,
        ...
        hor_probs, ver_probs, shoot_probs, target_probs, _, policy_state, next_hiddens = self.macro_forward(global_state,
                                                                                              native_state,
                                                                                              token_state,
        ...
        return hors, vers, shoots, targets, v_c, nn_c, next_hiddens

    def batch_forward(self, global_state, native_state, token_state, self_msl_token_state, bandit_msk_token_state,
                      ids_one_hot_state, hor_masks, ver_masks, shoot_masks, target_masks, hor_one_hots,
                      ver_one_hots, hidden_h, hidden_c):
        # function used for training #
        hor_probs, ver_probs, shoot_probs, target_probs, v_head, policy_state, next_hiddens = self.macro_forward(global_state,
                                                                                                   native_state,
                                                                                                   token_state,
                                                                                                   self_msl_token_state,
                                                                                                   bandit_msk_token_state,
        ...
        v_c_probs, nn_c_probs = self.hybrid_forward(policy_state, hor_one_hots, ver_one_hots)
        return hor_probs, ver_probs, shoot_probs, target_probs, v_c_probs, nn_c_probs, v_head

    def get_log_prob_and_values(self, x0, x1, x2, x3, x4, x5, hors, vers, shoots, targets, hor_masks, ver_masks,
                                shoot_masks, target_masks, hor_one_hots, ver_one_hots, v_cs, nn_cs, hidden_h, hidden_c):

        hor_probs, ver_probs, shoot_probs, target_probs, v_c_probs, nn_c_probs, value = self.batch_forward(
            x0, x1, x2, x3, x4, x5, hor_masks, ver_masks, shoot_masks, target_masks, hor_one_hots, ver_one_hots, hidden_h, hidden_c)
