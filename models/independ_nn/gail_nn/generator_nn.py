...
class Independ_Semantic_NN(nn.Module):
    ...
    def macro_forward(self, global_state, native_state, token_state, self_msl_token_state, bandit_msl_token_state,
                      ids_one_hot_state, hor_mask, ver_mask, shoot_mask, target_mask):
        ...
        return hor_probs, ver_probs, shoot_probs, target_probs, v_head, policy_state
    ...
    def select_action(self, global_state, native_state, token_state, self_msl_token_state, bandit_msl_token_state,
        ...
        hor_probs, ver_probs, shoot_probs, target_probs, _, policy_state = self.macro_forward(global_state,
                                                                                              native_state,
                                                                                              token_state,
        ...
        return hors, vers, shoots, targets, v_c, nn_c
    ...
    def batch_forward(self, global_state, native_state, token_state, self_msl_token_state, bandit_msl_token_state,
        ...
        hor_probs, ver_probs, shoot_probs, target_probs, v_head, policy_state = self.macro_forward(global_state,
                                                                                                   native_state,
                                                                                                   token_state,
                                                                                                   self_msl_token_state,
                                                                                                   bandit_msl_token_state,
    ...
    def supervise_forward(self, device, global_state,
                          native_state,
                          token_state,
                          self_msl_token_state,
                          bandit_msl_token_state,
        ...
        shoot_exp = torch.exp(self.shoot_heads(s_hiddens))
        target_exp = torch.exp(self.target_heads(t_hiddens))
        shoot_probs = (shoot_exp * shot_mask) / torch.sum(shoot_exp * shot_mask + self.multinomial_protect, dim=-1, keepdim=True)
        target_probs = (target_exp * target_mask) / torch.sum(target_exp * target_mask + self.multinomial_protect, dim=-1, keepdim=True)

        # for v_c and nn_c
        hor_probs_r = hor_probs.view(-1, hor_probs.size(-1))
        ver_probs_r = ver_probs.view(-1, ver_probs.size(-1))

        hors = (hor_probs_r + self.multinomial_protect).multinomial(1)
        vers = (ver_probs_r + self.multinomial_protect).multinomial(1)

        hor_one_hots = torch.zeros(hor_probs_r.shape[0], hor_probs_r.shape[-1]).to(device).scatter_(1, hors, 1).view(-1,
                                                                                                                     2,
                                                                                                                     hor_probs_r.shape[
                                                                                                                         -1])
        ver_one_hots = torch.zeros(ver_probs_r.shape[0], ver_probs_r.shape[-1]).to(device).scatter_(1, vers, 1).view(-1,
                                                                                                                     2,
                                                                                                                     ver_probs_r.shape[
                                                                                                                         -1])

        # hor_one_hots = [index_to_one_hot(hors.tolist()[i][0], hor_probs.size(-1)) for i in range(hors.shape[0])]
        # hor_one_hots = torch.tensor(hor_one_hots).view(-1, 2, hor_probs.size(-1)).detach().to(device)
        #
        # ver_one_hots = [index_to_one_hot(vers.tolist()[i][0], ver_probs.size(-1)) for i in range(vers.shape[0])]
        # ver_one_hots = torch.tensor(ver_one_hots).view(-1, 2, ver_probs.size(-1)).detach().to(device)

        v_c_probs, nn_c_probs = self.hybrid_forward(policy_state, hor_one_hots, ver_one_hots)

        return hor_probs, ver_probs, shoot_probs, target_probs, v_c_probs, nn_c_probs

    def get_log_prob_and_values(self, x0, x1, x2, x3, x4, x5, hors, vers, shoots, targets, hor_masks, ver_masks,
                                shoot_masks, target_masks, hor_one_hots, ver_one_hots, v_cs, nn_cs):
