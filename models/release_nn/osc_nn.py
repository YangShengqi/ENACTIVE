...
class OSC_NN(nn.Module):
    def __init__(self, id_dim, ground_truth_dim, native_dim, state_token_dim, state_token_num,
        ...
        # oscillation
        mu_hidden_size = 64
        mu_input_dim = last_decision_dim + last_policy_dim
        self. mu_hiddens_0 = nn.Linear(mu_input_dim, mu_hidden_size)
        self.mu_hiddens_1 = nn.Linear(mu_hidden_size, mu_hidden_size)
        self.mu_heads = nn.Linear(mu_hidden_size, 1)  # weight, 1 dim, [-1, 1]
        ...
        last_value_dim = flatten_dim_with_global + last_decision_dim
        ...
        set_init([self.mu_hiddens_0], method=init_method)
        set_init([self.mu_hiddens_1], method=init_method)
        set_init([self.mu_heads], method=init_method)
    ...
    def macro_forward(self, global_state, native_state, token_state, self_msl_token_state, bandit_msk_token_state,
        ...
        if self.is_lstm:
            ...
            policy_state = policy_state_v
        else:
            policy_state = comm_state[:, :, :]
        ...
        # ------------------------oscillation-------------------------------------
        mu_state = torch.cat((policy_state, last_decision), dim=-1)
        mu_hiddens = self.activation(self.mu_hiddens_0(mu_state))
        mu_hiddens = self.activation(self.mu_hiddens_1(mu_hiddens))
        mu = self.activation(self.mu_heads(mu_hiddens))  # [-1, 1]
        picmu = (mu + 1) / 2  # [0, 1]

        hor_dim = self.action_dims['horizontal_cmd_dim']
        ver_dim = self.action_dims['vertical_cmd_dim']
        target_dim = self.action_dims['target_dim']

        target_last_probs = last_decision[:, :, [i for i in range(target_dim)]]
        hor_last_probs = last_decision[:, :, [i for i in range(target_dim, target_dim + hor_dim)]]
        ver_last_probs = last_decision[:, :, [i for i in range(target_dim + hor_dim, target_dim + hor_dim + ver_dim)]]

        target_probs = picmu * target_last_probs + (1 - picmu) * target_probs
        hor_probs = picmu * hor_last_probs + (1 - picmu) * hor_probs
        ver_probs = picmu * ver_last_probs + (1 - picmu) * ver_probs
        # ----------------------------------------------------------------------------
        ...
        # value_net input
        value_state = torch.cat((value_state, last_decision), dim=-1)
        ...
        return hor_probs, ver_probs, shoot_probs, target_probs, v_head, policy_state, lts_probs, next_hiddens, picmu

    def hybrid_forward(self, policy_state, hor_c_one_hots, ver_c_one_hots, last_decision, picmu):
        ...
        ver_dim = self.action_dims['vertical_cmd_dim']
        target_dim = self.action_dims['target_dim']
        v_c_dim = self.action_dims['v_c_dim']
        nn_c_dim = self.action_dims['nn_c_dim']

        v_c_last_probs = last_decision[:, :,
                         [i for i in range(target_dim + hor_dim + ver_dim, target_dim + hor_dim + ver_dim + v_c_dim)]]
        nn_c_last_probs = last_decision[:, :, [i for i in range(target_dim + hor_dim + ver_dim + v_c_dim,
                                                             target_dim + hor_dim + ver_dim + v_c_dim + nn_c_dim)]]
        v_c_probs = picmu * v_c_last_probs + (1 - picmu) * v_c_probs
        nn_c_probs = picmu * nn_c_last_probs + (1 - picmu) * nn_c_probs
        # --------------------------------------------------------
    ...
    def select_action(self, global_state, native_state, token_state, self_msl_token_state, bandit_msk_token_state,
        ...
        last_decision = torch.FloatTensor(last_decision).unsqueeze(0)
        ...
        hor_probs, ver_probs, shoot_probs, target_probs, _, policy_state, lts_probs, next_hiddens, picmu = self.macro_forward(
            global_state,
            native_state,
            token_state,
        ...
        hor_one_hots = torch.tensor(hor_one_hots).detach().unsqueeze(0)
        ver_one_hots = [index_to_one_hot(vers.tolist()[i][0], ver_probs.size(1))
                        for i in range(self.aircraft_num)]
        ver_one_hots = torch.tensor(ver_one_hots).detach().unsqueeze(0)

        v_c_probs, nn_c_probs = self.hybrid_forward(policy_state, hor_one_hots, ver_one_hots, last_decision, picmu)

        v_c_probs = v_c_probs.squeeze(0)
        nn_c_probs = nn_c_probs.squeeze(0)

        v_c = (v_c_probs + self.multinomial_protect).multinomial(1)
        nn_c = (nn_c_probs + self.multinomial_protect).multinomial(1)

        lts_probs = lts_probs.squeeze(0)
    ...
    def batch_forward(self, global_state, native_state, token_state, self_msl_token_state, bandit_msk_token_state,
        ...
        hor_probs, ver_probs, shoot_probs, target_probs, v_head, policy_state, lts_prob, next_hiddens, picmu = self.macro_forward(
            global_state,
            native_state,
            token_state,
            self_msl_token_state,
...
if __name__ == "__main__":
    ...
    model = LTS_Force_Interupt_LSTM_NN(Config.env.red, len(red_state_global[0]), len(red_atten_state[0][0]),
