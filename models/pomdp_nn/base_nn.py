...
class Base_NN(nn.Module):
    def __init__(self, id_dim, ground_truth_dim, native_dim, state_token_dim, state_token_num,
                 ...
                 bandit_predict_state_dim, bandit_predict_num,
                 seq_len=4,
                 is_lstm=True,
                 action_dims=dict(horizontal_cmd_dim=0, vertical_cmd_dim=0, shoot_dim=0, target_dim=0, v_c_dim=0, nn_c_dim=0,bandit_predict_state_dim=0),
                 ground_truth_size_before_cat=(256, 256),
                 native_hidden_size=(256, 128),
                 policy_hidden_size=(128, 128, 128),
                 value_hidden_size=(128, 64, 1),
        ...
        self.bandit_predict_dim = bandit_predict_state_dim
        self.bandit_predict_num = bandit_predict_num
        #  ---------------------------------actor -----------------------------------------------------#
        flatten_dim_without_global = self.state_token_embed_dim * state_token_num + \
                                     self.msl_token_embed_dim * self_msl_token_num + \
                                     last_native_dim + id_dim + self.bandit_predict_dim * self.bandit_predict_num
        ...
        # ---------------------------------------------predict--------------------------------------------- #
        # todo for only 2V2
        predict_head_hidden_size = int(last_policy_dim * self.aircraft_num / 4)
        bandit_predict_out_dim = int(len(action_dims["bandit_predict_out_dim"]) / 2)  # todo 6 = x3 + v3

        self.bandit_predict_hiddens = nn.Linear(last_policy_dim * self.aircraft_num, predict_head_hidden_size)

        self.bandit_predict_hiddens_0 = nn.Linear(predict_head_hidden_size, predict_head_hidden_size)
        self.bandit_predict_x_heads_0 = nn.Linear(predict_head_hidden_size, bandit_predict_out_dim)
        self.bandit_predict_v_heads_0 = nn.Linear(predict_head_hidden_size, bandit_predict_out_dim)

        self.bandit_predict_hiddens_1 = nn.Linear(predict_head_hidden_size, predict_head_hidden_size)
        self.bandit_predict_x_heads_1 = nn.Linear(predict_head_hidden_size, bandit_predict_out_dim)
        self.bandit_predict_v_heads_1 = nn.Linear(predict_head_hidden_size, bandit_predict_out_dim)
        ...
        # for pomdp
        set_init([self.bandit_predict_hiddens], method=init_method)
        set_init([self.bandit_predict_hiddens_0], method=init_method)
        set_init([self.bandit_predict_hiddens_1], method=init_method)
        set_init([self.bandit_predict_x_heads_0], method=init_method)
        set_init([self.bandit_predict_v_heads_1], method=init_method)
        set_init([self.bandit_predict_x_heads_0], method=init_method)
        set_init([self.bandit_predict_v_heads_1], method=init_method)

    def macro_forward_with_bandit_msl(self, global_state, native_state, token_state, self_msl_token_state, bandit_msk_token_state,
    ...
    def macro_forward(self, global_state, native_state, token_state, self_msl_token_state,
                      ids_one_hot_state, hor_mask, ver_mask, shoot_mask, target_mask, last_decision, lts_mask,
                      deliberation_cost, hidden_h, hidden_c, bandit_predict):
        ...
        cat_state = torch.cat(
            [ids_one_hot_state, native_state, state_tokens_out_flat, self_msl_tokens_out_flat, torch.flatten(bandit_predict, start_dim=-2, end_dim=-1)], dim=-1)    # batch_size * num_aircraft * seq_len * feature_dim #

        comm_state = self.policy_commnet(cat_state)   # batch_size * num_aircraft * (seq_len == 1  *) feature_dim #
        ...
        if self.is_lstm:
            ...
            policy_state_v = policy_state[:, -1, :].view(batch_size, self.aircraft_num, -1)
            policy_state = policy_state_v.squeeze(0)
            bandit_predict_state = torch.flatten(policy_state_v, -2, -1)
        else:
            policy_state_v = comm_state[:, :, -1, :]
            policy_state = comm_state[:, :, :].squeeze(0)
            bandit_predict_state = torch.flatten(policy_state_v, -2, -1)

        # *** bandit_predict *** #
        # print(bandit_predict_state)

        bandit_predict_hidden_state = self.activation(self.bandit_predict_hiddens(bandit_predict_state))

        bandit_predict_0 = self.activation(self.bandit_predict_hiddens_0(bandit_predict_hidden_state))
        bandit_predict_x_0 = self.activation(self.bandit_predict_x_heads_0(bandit_predict_0))
        bandit_predict_v_0 = self.activation(self.bandit_predict_v_heads_0(bandit_predict_0))

        bandit_predict_1 = self.activation(self.bandit_predict_hiddens_1(bandit_predict_hidden_state))
        bandit_predict_x_1 = self.activation(self.bandit_predict_x_heads_1(bandit_predict_1))
        bandit_predict_v_1 = self.activation(self.bandit_predict_v_heads_1(bandit_predict_1))

        bandit_predict_out_0 = torch.cat([bandit_predict_x_0, bandit_predict_v_0], -1)
        bandit_predict_out_1 = torch.cat([bandit_predict_x_1, bandit_predict_v_1], -1)
        ...
        if self.is_lstm:
            ...
            value_state = torch.cat(
                [ids_one_hot_state[:, :, -1, :], global_state, comm_state], dim=-1)
        for affine in self.value_affine_layers:
            value_state = affine(value_state)
            value_state = self.activation(value_state)
        ...
        return hor_probs, ver_probs, shoot_probs, target_probs, v_head, policy_state, lts_probs, next_hiddens, bandit_predict_out_0, bandit_predict_out_1
    ...
    def select_action(self, global_state, native_state, token_state, self_msl_token_state,
                      ids_one_hot_state, hor_masks: list, ver_masks: list, shoot_masks: list, target_masks: list,
                      last_decision, lts_masks, deliberation_cost, hidden_h, hidden_c, bandit_predict):
        ...
        bandit_predict = torch.FloatTensor(bandit_predict).unsqueeze(0)   # batch * air_num * len * bandit_num * 6
        ...
        hor_probs, ver_probs, shoot_probs, target_probs, _, policy_state, lts_probs, next_hiddens, bandit_predict_0, bandit_predict_1 = self.macro_forward(
            global_state,
            native_state,
            token_state,
        ...
        return hors, vers, shoots, targets, v_c, nn_c, lts, next_hiddens, [bandit_predict_0, bandit_predict_1]

    def batch_forward(self, global_state, native_state, token_state, self_msl_token_state,
                      ids_one_hot_state, hor_masks, ver_masks, shoot_masks, target_masks, hor_one_hots, ver_one_hots,
                      last_decision, lts_masks, deliberation_cost, hidden_h, hidden_c, bandit_predict_state):
        # function used for training #
        hor_probs, ver_probs, shoot_probs, target_probs, v_head, policy_state, lts_prob, next_hiddens, bandit_predict_0, bandit_predict_1 = self.macro_forward(
            global_state,
            native_state,
            token_state,
            self_msl_token_state,
            ...
            ver_masks,
            shoot_masks,
            target_masks, last_decision, lts_masks, deliberation_cost, hidden_h, hidden_c, bandit_predict_state)

        v_c_probs, nn_c_probs = self.hybrid_forward(policy_state, hor_one_hots, ver_one_hots)
        return hor_probs, ver_probs, shoot_probs, target_probs, v_c_probs, nn_c_probs, v_head, lts_prob, bandit_predict_0, bandit_predict_1

    def forward(self, x):
        pass

    def get_log_prob_and_values(self, x0, x1, x2, x3, x4, hors, vers, shoots, targets, hor_masks, ver_masks,
                                shoot_masks, target_masks, hor_one_hots, ver_one_hots, v_cs, nn_cs, last_decision,
                                lts_masks, lts_choice, deliberation_cost, hidden_h, hidden_c, bandit_predict_state):

        hor_probs, ver_probs, shoot_probs, target_probs, v_c_probs, nn_c_probs, value, lts_probs, bandit_predict_0, bandit_predict_1 = self.batch_forward(
            x0, x1, x2, x3, x4, hor_masks, ver_masks, shoot_masks, target_masks, hor_one_hots, ver_one_hots,
            last_decision, lts_masks, deliberation_cost, hidden_h, hidden_c, bandit_predict_state)
        
        # for train
        ret_for_loss = [hor_probs, ver_probs, shoot_probs, target_probs, v_c_probs, nn_c_probs, value, lts_probs, bandit_predict_0, bandit_predict_1]
        ...
        return ans, value, ret_for_loss


if __name__ == "__main__":
    from environment.lts_state_method import get_kteam_global_ground_truth_state, \
        get_kteam_aircraft_state_for_attention, get_self_kteam_msl_tokens, get_bandit_kteam_msl_tokens, \
        get_kteam_ids_one_hot_state

    red_state_global = get_kteam_global_ground_truth_state(Config.env, 0)
    red_atten_state = get_kteam_aircraft_state_for_attention(Config.env, 0)
    msl_token_self = get_self_kteam_msl_tokens(Config.env, 0)
    msl_token_bandit = get_bandit_kteam_msl_tokens(Config.env, 0)
    state_id_one_hot = get_kteam_ids_one_hot_state(Config.env, 2)
    ...
    model = Base_NN(Config.env.red, len(red_state_global[0]), len(red_atten_state[0][0]),
    ...
    native_state = red_atten_state[0]
    token_state = red_atten_state[1]
    self_msl_token_state = msl_token_self
    bandit_msk_token_state = msl_token_bandit
    ...
    # last_decision = zip([last_target, last_hor, last_ver])
    last_decision = []
    for i in range(2):
        last_decision.append(last_target[i] + last_hor[i] + last_ver[i] + last_v[i] + last_nn[i])

    hors, vers, shoots, targets, v_c, nn_c, lts = model.select_action(red_state_global, native_state, token_state,
                                                                      self_msl_token_state, bandit_msk_token_state,
                                                                      state_id_one_hot, hor_masks, ver_masks,
                                                                      shoot_masks, target_masks, last_decision,
                                                                      lts_mask, [[0, 0.4], [0, 0.4]])

    print(hors, vers, shoots, targets, v_c, nn_c, lts)
    #
    # hor_masks = [[1] * 6] * 2
    # ver_masks = [[1] * 6] * 2